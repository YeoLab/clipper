...
def adjust_offsets(tool, offsets=None):
    ...
    For finding motif position relative to center of peaks
    ...
    """
    #TODO This function depends on bed12, fix to work for bed6
    if offsets is None:
...
def count_genomic_region_sizes(regions_dir, regions, species="hg19"):
    ...
    #TODO update this to work off GFF file, because something isn't matching up...
...
def assign_to_regions(tool, clusters, speciesFA, regions_dir, regions,
    ...
    tool - a bed tool (each line representing a cluster)
    ...
    #TODO Refactor, can't assume bed12
    ...
    for region in regions:
        ...
        region_fa = fa_file(clusters, region = region, directory=fasta_dir, genic_type="real")
        ...
        write_seqs(fa_file(clusters, region = region, directory=fasta_dir, genic_type="random"), rand_list)
    ...
    real_fa = fa_file(clusters, region = "all", directory=fasta_dir, genic_type="real")
    ...
    write_seqs(fa_file(clusters, region = "all", directory=fasta_dir, genic_type="random"), rand_list)
...
def calculate_peak_locations(bedtool, genes, regions, features):
    ...
    #gets rna position for every line as well
    ...
    #we will know if this is a problem if there a lot of ignoring error messages
...
def cluster_peaks(bedtool, bw_pos, bw_neg, k=16):
    ...
    #TODO This is slow, switch to just sorting
    bed_center = bedtool.each(small_peaks).saveas()
...
def run_homer(foreground, background, k=list([5, 6, 7, 8, 9]), out_loc=os.getcwd()):
    ...
    try:
        with open(os.devnull, 'w') as fnull:
            result = subprocess.call(["findMotifs.pl", foreground, "fasta", out_loc, "-nofacts", "-p", "4", "-rna",
                                      "-S", "20", "-len", k, "-noconvert", "-nogo", "-fasta", background],
                                     shell=False, stdout=fnull)
            
        print "Homer Finished, output here: %s" % out_loc
    except OSError:
...
def chop(chrom, start, end, wsize=5):
    ...
    i = start
    while i < end:
        x = pybedtools.Interval(chrom, i, (i+wsize))
        p = get_mean_phastcons(x, species="hg19")
        file.write("\t".join(map(str, [chrom, i, (i+wsize-1), p])) + "\n")
        i += wsize
    file.close()
...
def get_mean_phastcons(bedtool, phastcons_location, sample_size=1000):
    ...
    with open(phastcons_location) as bw_file:
        ...
        for bed_line in bedtool.random_subset(min(len(bedtool), sample_size)):
            conservation_values = bw.get_as_array(bed_line.chrom, bed_line.start, bed_line.stop)
            try:
...
def fa_file(filename, region=None, directory=None, genic_type="real"):
    ...
    if region is not None:
        full_name = "%s.%s.%s.fa" % (filename, region, genic_type)
        return os.path.join(directory, full_name)
    else:
        full_name = "%s.%s.fa" % (filename, genic_type)
        return os.path.join(directory, full_name)
...
def count_reads_per_cluster(bedtool, bamtool):
    ...
    try:
        ...
        print "output not from clipper, reverting to bedtools"
...
def calculate_kmer_diff(kmer_list, regions, clusters, fasta_dir):
    ...
    fasta_dir - str directory of fasta files
    ...
    for region in regions:
        ...
        real_fa = fa_file(clusters, region=region, directory=fasta_dir, genic_type="real")
        rand_fa = fa_file(clusters, region=region, directory=fasta_dir, genic_type="random")
...
def calculate_homer_motifs(kmer_list, regions, clusters, fasta_dir, homer_out):
    
    """
    
    Calculates motifs for homer
    
    kmer_list - list[int] different kmers to examine
    regions   - list[str] regions to compute kmers on
    clusters - str name out output file
    fasta_dir - str directory of fasta files
    homer_out - str output dir of homer
    ...
    for region in regions:
        ...
        region_homer_out = os.path.join(homer_out, region)
        run_homer(real_fa, rand_fa, kmer_list, out_loc=region_homer_out)
...
def calculate_phastcons(regions, cluster_regions, phastcons_location, sample_size=1000):
    ...
    regions - list[str] regions to analyze
    ...
    sample_size - int the number of regions to randomly sample to calculate phastcons (needed because sampling from bw
    file is slow for phastcons)
    
    """

    #TODO should use multiprocessing to speed this part up!
    phast_values = {"real" : {}, "rand" : {}}
    print "Fetching Phastcons Scores..."

    #phastcons values for all regions except "all", skip "all" combine them later
    for region in regions:
        print "%s..." % (region)
        ...
        try:
            ...
            #can't concatenate zero length arrays, so prime it
            randPhast = np.array([])
...
def calculate_motif_distance(cluster_regions, region_sizes, motif_tool, slopsize = 200):
    ...
    for region in cluster_regions.keys():
        ...
        dist_dict[region]['real']['dist'] = get_motif_distance(cluster_regions[region]['real'],
                                                               motif_tool, slop=slopsize)
        ...
        for i in range(nrand):
            dist_dict[region]['rand']['dist'] += get_motif_distance(cluster_regions[region]['rand'][i],
                                                                    motif_tool, slop=slopsize)
...
def get_motif_distance(clusters, motif, slop=500):
    ...
    #TODO This assumes bed12 refactor
...
def make_unique(bedtool):
    for unique_id, interval in enumerate(bedtool):
        interval.name = interval.name + "_" + str(unique_id)
        yield interval
...
def main(options):
    ...
    clusters_bed = pybedtools.BedTool(make_unique(pybedtools.BedTool(options.clusters))).saveas()
    ...
    assigned_dir = os.path.join(out_dir, "assigned")
    misc_dir = os.path.join(out_dir, "misc")
    fasta_dir = os.path.join(out_dir, "fasta")
    homer_out_base = os.path.join(out_dir, "homer")
    make_dir(homer_out_base)
    homer_out = os.path.join(homer_out_base, clusters)
    ...
    #all category would break some analysis, create copy and remove it
    ...
    cluster_regions, region_sizes = assign_to_regions(clusters_bed, clusters, options.genome_location,
                                                      options.regions_location, assigned_regions,
                                                      assigned_dir=assigned_dir, fasta_dir=fasta_dir,
                                                      species=species, getseq=True, nrand=options.nrand,)

    genic_region_sizes = count_genomic_region_sizes(options.regions_location, assigned_regions, species)
    ...
    cluster_lengths = bed_lengths(cluster_regions['all']['real'])
    print "done"
    ...
    (cluster_types, premRNA_positions, mRNA_positions, exon_positions,
    ...
    read_densities, classes = cluster_peaks(cluster_regions['all']['real'], bw_pos, bw_neg)

    #generates figure 10 (exon distances)
    cluster_type_count = [cluster_types["CE:"], cluster_types["SE:"],
                  cluster_types["MXE:"], cluster_types["A5E:"], cluster_types["A3E:"]]
    ...
    if options.homer:
        calculate_homer_motifs(options.k, regions, clusters, fasta_dir, homer_out)
    ...
    out_dict["cluster_type_count"] = cluster_type_count
    ...
    out_dict['homer_out'] = homer_out
    out_dict['regions'] = homer_out
    ...
    QCfig_params = [reads_in_clusters, (total_reads - reads_in_clusters), 
                    ...
                    genomic_type_count, cluster_type_count, homer_out, kmer_results, motifs, phast_values,
                    regions, np.array(read_densities), classes]
    
    qc_fig = CLIP_analysis_display.CLIP_QC_figure(*QCfig_params)
    distribution_fig = CLIP_analysis_display.plot_distributions(features_transcript_closest,
                                                                features_mrna_closest, distributions)
    qc_fig.savefig(os.path.join(out_dir, clusters + ".qc_fig." + options.extension))
    distribution_fig.savefig(os.path.join(out_dir, clusters + ".DistFig." + options.extension))
