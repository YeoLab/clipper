...
def get_FDR_cutoff_mode(readlengths,
    ...
    while bad == 1 and tries < 5:
        try:
            process = Popen([cmd, "-f", "stdin", "-L",
                             str(genelength), "-r", str(iterations),
                             "-a", str(alpha)], stdin=PIPE, stdout=PIPE)
...
class SmoothingSpline(PeakGenerator):
    ...
    def get_norm_penalized_residuals(self, spline, norm_weight = 1, residual_weight = 10):
        ...
        """
        #this import needs to be here for whatever reason
        from scipy.linalg import norm
    ...
    def optimize_fit(self, s_estimate=None, method = 'L-BFGS-B', bounds=((1,None),),
        ...
        minimizeResult = scipy.optimize.minimize(self.fit_loss, s_estimate, args=weight,
                                                 options=minOpts, method=method, bounds=bounds,
                                                 )
    ...
    def peaks(self, threshold=0, plotit = False):
        ...
        try:
            ...
            #low-temp optimize
            optimized_spline = self.optimize_fit(s_estimate=bestSmoothingEstimate)
        ...
        spline_values = array([int(x) for x in optimized_spline(self.xRange)])

        if plotit is True:
            self.plot()

        starts_and_stops, starts, stops = self.get_regions_above_threshold(self.threshold, spline_values)
        peak_definitions = []
...
def plot_sections(wiggle, sections, threshold):
    ...
    for start, stop in sections:
        ax.plot(np.arange(start, stop), wiggle[start:stop], color='g')
    return fig
...
def peaks_from_info(wiggle, pos_counts, lengths, interval, gene_length,
    ...
    if user_threshold is None:
        #Uses Binomial Distribution to get cutoff if specified by user
        if method == "binomial":
    ...
    for sect in sections:
        ...
        if user_threshold is None:
            if SloP:
                ...
                if method == "binomial":
                    threshold = max(gene_threshold,
                                    get_FDR_cutoff_binom(sect_read_lengths,
                                                         sect_length,
                                                         binom_alpha))
                elif method == "random":
                    #use the max FDR cutoff between superlocal and gene-wide calculations
                    threshold = max(gene_threshold,
        ...
        if max(data) < threshold:
            logging.info("data does not exceed threshold, stopping")
            continue
        ...
        try:
            ...
            logging.info("I identified %d potential peaks" % num_peaks)
